...
class EnhancedRedisConnection:
    ...
    async def health_check(self) -> bool:
        if self.redis is None:
            ...
            logger.warning(
                f"[{self.name}] Health check failed: redis not initialized, "
                f"failures={self.stats.health_check_failures}"
            )
            return False
        ...
        try:
            ...
            self.stats.health_check_failures += 1
            self.stats.last_health_check = time.time()
            logger.warning(f"[{self.name}] Health check cancelled")
        except Exception as e:
            self.stats.health_check_failures += 1
            self.stats.last_health_check = time.time()
            self.stats.last_error = str(e)
    ...
    def is_available(self) -> bool:
        return self.redis is not None and self.stats.initialized
